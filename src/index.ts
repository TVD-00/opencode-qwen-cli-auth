...
const ENABLE_CLI_FALLBACK =
  process.env.OPENCODE_QWEN_ENABLE_CLI_FALLBACK === "1";
...
async function applyRequestTiming(): Promise<void> {
  ...
  timingChain = new Promise<void>((r) => {
    resolveMine = r;
  });
  try {
    ...
    if (lastRequestTimestamp > 0 && elapsed < INTER_REQUEST_GAP_MS) {
      ...
      const jitter = randomDelay(
        PRE_REQUEST_JITTER_MIN_MS,
        PRE_REQUEST_JITTER_MAX_MS,
      );
...
function makeFailFastErrorResponse(
  status: number,
  code: string,
  message: string,
): Response {
...
function getRequestIdleTimeoutMs(): number {
...
interface RequestActivityGuard {
  signal: AbortSignal;
  reset: () => void;
  cleanup: () => void;
  abortedReason: () => Error | null;
}

function createRequestActivityGuard(
...
function wrapStreamingResponseWithActivityTimeout(
  response: Response,
  guard: RequestActivityGuard,
): Response {
  if (!response.body) {
    guard.cleanup();
    return response;
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const finish = (action: "close" | "error", value?: unknown) => {
        if (settled) {
          return;
        }
        settled = true;
...
async function normalizeFetchInvocation(
  ...
  let requestInput: string = isRequestInstance(input)
    ? input.url
    : (input as string);
...
function getHeaderValue(
  ...
  if (headers instanceof Headers) {
    return (
      headers.get(headerName) ?? headers.get(normalizedHeader) ?? undefined
    );
  }
  if (Array.isArray(headers)) {
    const pair = headers.find(
      ([name]) => String(name).toLowerCase() === normalizedHeader,
    );
...
function applyAuthorizationHeader(
  ...
  if (Array.isArray(requestInit.headers)) {
    const existing = requestInit.headers.findIndex(
...
function rewriteRequestBaseUrl(
  requestInput: string,
  resourceUrl: string,
): string {
  if (
...
function applyJsonRequestBody(
  ...
  if (Array.isArray(requestInit.headers)) {
    const hasContentType = requestInit.headers.some(
...
function parseJsonRequestBody(requestInit: RequestInit): RequestPayload | null {
  ...
  const contentType = getHeaderValue(
    requestInit.headers as HeadersInput,
    "content-type",
  );
...
function sanitizeOutgoingPayload(payload: RequestPayload): RequestPayload {
  ...
  if (
    typeof sanitized.max_tokens === "number" &&
  ...
  if (
    typeof sanitized.max_completion_tokens === "number" &&
 ...
 */
function createQuotaDegradedPayload(
  payload: RequestPayload,
): RequestPayload | null {
  ...
  // Reduce max_tokens
  if (
    typeof degraded.max_tokens !== "number" ||
  ...
  if (
    typeof degraded.max_completion_tokens === "number" &&
 ...
 */
function payloadContainsNonTextMessages(
...
function buildQwenCliPrompt(payload: RequestPayload | null): string {
  ...
  const merged = messages
    ...
    .map((message) => {
      ...
      const role =
        typeof msg?.role === "string" ? msg.role.toUpperCase() : "UNKNOWN";
      return `${role}: ${text}`;
    })
    .filter(Boolean)
    .join("\n\n");
  ...
  return await new Promise<CliFallbackResult>((resolve) => {
    ...
    child.on("close", (exitCode: number | null) => {
      ...
      if (content) {
        finalize({
          ok: true,
          response: makeQwenCliCompletionResponse(
            model,
            content,
            context,
            streamMode,
 ...
 */
function makeQuotaFailFastResponse(
  text: string,
 ...
 */
async function sendWithActivityTimeout(
  input: string,
  requestInit: RequestInit,
  expectStreaming: boolean,
): Promise<Response> {
  const guard = createRequestActivityGuard(
    requestInit.signal,
    getRequestIdleTimeoutMs(),
  );
  try {
...
function applyDashScopeHeaders(requestInit: RequestInit): void {
  ...
  if (Array.isArray(requestInit.headers)) {
    const existing = new Set(
      (requestInit.headers as [string, string][]).map(([name]) =>
  ...
  const existingKeys = new Set(
    Object.keys(requestInit.headers as Record<string, string>).map((k) =>
      k.toLowerCase(),
    ),
  );
  for (const [key, value] of Object.entries(headersToApply)) {
...
async function failFastFetch(
  ...
  try {
    ...
    let response = await sendWithActivityTimeout(
      requestInput,
      requestInit,
      payload?.stream === true,
    );
        ...
        if (response.status === 401) {
          ...
          try {
            ...
            if (freshAccount?.accessToken) {
              ...
              response = await sendWithActivityTimeout(
                requestInput,
                requestInit,
                payload?.stream === true,
              );
          ...
          if (payload && isInsufficientQuota(firstBody)) {
            if (context.accountID) {
              try {
                ...
                logWarn(
                  "Failed to switch OAuth account after insufficient_quota",
                  switchError,
                );
              }
            }
    ...
    if (
      ...
      return makeFailFastErrorResponse(
        400,
        "request_timeout",
        `Qwen request stalled for more than ${idleTimeoutMs}ms`,
      );
    }
...
export const QwenAuthPlugin: Plugin = async (_input) => {
  return {
      ...
      async loader(getAuth, provider) {
        ...
        if (provider?.models) {
          for (const model of Object.values(provider.models)) {
            if (model)
              (model as { cost?: { input: number; output: number } }).cost = {
                input: 0,
                output: 0,
              };
          }
        ...
        return {
          ...
          fetch: (input: RequestInfo | URL, init?: RequestInit) =>
            failFastFetch(input as Request | string, init, currentAccountId),
        };
      },
      methods: [
        {
              ...
              callback: async () => {
                ...
                while (Date.now() - startTime < expiresIn) {
                  await new Promise((resolve) =>
                    setTimeout(resolve, pollInterval + POLLING_MARGIN_MS),
                  );
                  ...
                  if (result.type === "failed") {
                    ...
                    logWarn(
                      `OAuth token polling failed (${consecutivePollFailures}/${MAX_CONSECUTIVE_POLL_FAILURES})`,
                    );
                    if (
                      consecutivePollFailures >= MAX_CONSECUTIVE_POLL_FAILURES
                    ) {
                      console.error(
                        "[qwen-oauth-plugin] OAuth token polling failed repeatedly",
                      );
                      return { type: "failed" as const };
                    }
                    continue;
                  ...
                  if (result.type === "denied") {
                    console.error(
                      "[qwen-oauth-plugin] Device authorization was denied",
                    );
                    return { type: "failed" as const };
                  }
                  if (result.type === "expired") {
                    console.error(
                      "[qwen-oauth-plugin] Device authorization code expired",
                    );
                    return { type: "failed" as const };
                  }
                  return { type: "failed" as const };
                ...
                console.error(
                  "[qwen-oauth-plugin] Device authorization timed out",
                );
                return { type: "failed" as const };
              },
            };
        ...
        },
        {
          ...
          authorize: async () => {
            ...
            return {
              ...
              method: "auto" as const,
              instructions:
                "Login with a DIFFERENT Qwen account to add it as backup for auto-switch.",
              callback: async () => {
                let pollInterval = (deviceAuth.interval || 5) * 1000;
                const POLLING_MARGIN_MS = 3000;
                const maxInterval = DEVICE_FLOW.MAX_POLL_INTERVAL;
                ...
                while (Date.now() - startTime < expiresIn) {
                  ...
                  if (result.type === "success") {
                    ...
                    try {
                      ...
                      logWarn(
                        "Failed to restore active account after adding new account",
                      );
                    }
                    return {
                      type: "success" as const,
                  ...
                  if (result.type === "failed") {
                    if (result.fatal) {
                      logError(
                        "OAuth token polling failed with fatal error (add-account)",
                        {
                          status: result.status,
                          error: result.error,
                          description: result.description,
                  ...
                  if (result.type === "denied") {
                    console.error(
                      "[qwen-oauth-plugin] Device authorization was denied (add-account)",
                    );
                    return { type: "failed" as const };
                  }
                  if (result.type === "expired") {
                ...
                console.error(
                  "[qwen-oauth-plugin] Device authorization timed out (add-account)",
                );
                return { type: "failed" as const };
              },
            };
    ...
    config: async (config) => {
      const providers =
        (config as unknown as { provider?: Record<string, unknown> })
          .provider || {};
