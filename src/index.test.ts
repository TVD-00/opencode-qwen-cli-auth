...
describe("config hook — provider registration", () => {
  ...
  it("coder-model output limit is 65536 (CHAT_MAX_TOKENS_CAP)", async () => {
    ...
    expect(providerEntry.models["coder-model"].limit.output).toBe(
      CHAT_MAX_TOKENS_CAP,
    );
  });
...
describe("chat.params hook — max_tokens capping", () => {
  ...
  it("leaves options.timeout unchanged when larger", async () => {
    const plugin = await buildPlugin();
    const output: Record<string, unknown> = {
    ...
    await plugin["chat.params"]({}, output);
    const out = output as { options: Record<string, unknown> };
    expect(out.options.timeout).toBe(999_999_999);
  });
...
describe("chat.headers hook — header injection", () => {
  ...
  it("sets X-DashScope-UserAgent matching User-Agent", async () => {
    ...
    expect(out.headers["X-DashScope-UserAgent"]).toBe(
...
describe("payload sanitization — client-only fields stripped", () => {
  ...
  it("strips providerID from outgoing body", async () => {
    ...
    const sentBody = JSON.parse(calledInit.body as string) as Record<
      string,
      unknown
    >;
...
describe("loader — returns apiKey for SDK auth injection", () => {
  ...
  it("loader result disables provider total timeout", async () => {
    vi.mocked(getActiveOAuthAccount).mockResolvedValue({
    ...
    expect(loaderResult["timeout"]).toBe(false);
  });
});
...
// ---------------------------------------------------------------------------
// 16. Streaming timeout behavior — activity timeout instead of total timeout
// ---------------------------------------------------------------------------

describe("streaming activity timeout", () => {
  function makeTimedTextStream(
    ...
    return new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        if (typeof closeDelayMs === "number") {
          setTimeout(safeClose, elapsed + closeDelayMs);
        }
      },
    });
  }
  ...
  it("does not abort an active stream even when total duration exceeds 120s", async () => {
    vi.stubEnv("OPENCODE_QWEN_REQUEST_IDLE_TIMEOUT_MS", "50");
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(
          makeTimedTextStream(
            [
              { delayMs: 5, text: "chunk-1" },
              { delayMs: 30, text: "chunk-2" },
              { delayMs: 30, text: "chunk-3" },
            ],
            5,
          ),
          {
  ...
  it("aborts a stalled stream after 120s without new chunks", async () => {
    vi.stubEnv("OPENCODE_QWEN_REQUEST_IDLE_TIMEOUT_MS", "50");

    const fetchMock = vi.fn(
      async () =>
        new Response(makeTimedTextStream([{ delayMs: 5, text: "chunk-1" }]), {
          status: 200,
          headers: { "content-type": "text/event-stream; charset=utf-8" },
        }),
    );
    ...
    await expect(response.text()).rejects.toThrow("request_idle_timeout");
  }, 2_000);
});
